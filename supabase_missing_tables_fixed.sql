-- Script corrigido para criar tabelas que estão faltando no Supabase
...
-- 8. Criar função para atualizar updated_at automaticamente (se não existir)
...
-- 9. Criar triggers para atualizar updated_at (apenas se não existirem)
DO $$
BEGIN
    ...
    -- Trigger para financial_goals
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger 
        WHERE tgname = 'update_financial_goals_updated_at'
    ) THEN
...
-- 10. Criar função para gerar alertas de saldo baixo (se não existir)
...
-- 11. Criar trigger para verificar saldo baixo (apenas se não existir)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger 
        WHERE tgname = 'check_low_balance_trigger'
    ) THEN
...
-- 12. Criar função para verificar orçamentos excedidos (se não existir)
...
-- 13. Criar trigger para verificar orçamentos excedidos (apenas se não existir)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger 
        WHERE tgname = 'check_budget_exceeded_trigger'
    ) THEN
...
-- 14. Criar índices para melhor performance (apenas se não existirem)
...
SELECT 'Script executado com sucesso! Todas as tabelas e políticas foram criadas/verificadas.' as status; 